// PD downright point: key insight: must have different gains for going toward
// and away from downright point, due to aid of gravity.
// Tested and working as of 14 May on the new motor (with the torque) 
// And videotaped
...
void loop(){
    ...
    if (time_elapsed >= sample_time)
    {
        ...
        if (theta1 > 8) {
            if (theta1dot > 0.1) { // going away... slow it down
                /*motorWrite(someFlag * motor_output);*/
                motor.Fwd(150);
            }
            else if (theta1dot < 0.1) { // going towrad .. 
                /*motorWrite(-someFlag * motor_output);*/
                motor.Rev(60); // fight gravity harder
            }
        }
        else if (theta1 < -8) {
            ...
            else if (theta1dot < -0.1) { // going away
                /*motorWrite(someFlag * motor_output);*/
                motor.Rev(150);
...
// this one accidentally swings up
...
void setup() {
    ...
    /*setPwmFrequency(PWMPin,
     * 8);  // change Timer2
     * divisor to 8 gives 3.9kHz
     * PWM freq*/
}
            ...
            i++)
    {
        if
             ...
             '%')
            {
                ...
                Serial.write(reinterpret_cast<const
                        uint8_t*>(str+j),
                        i-j);
